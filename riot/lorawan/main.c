...
static void _send_message(void) {
  ...
  char payload[16];
  sprintf(payload, "%d %d %d %d %d", temperature, h, wd, wi, rh);
  printf(
      "Sending [TEMEPERATURE, HUMIDITY, WIND_DIRECTION, WIND_INTENSITY, "
      "RAIN_HEIGHT]: %s\n",
      payload);
  /* Try to send the message */
  uint8_t ret =
      semtech_loramac_send(&loramac, (uint8_t *)payload, strlen(payload));
